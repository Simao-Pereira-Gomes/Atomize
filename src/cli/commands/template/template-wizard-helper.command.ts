...
interface AssignmentPromptAnswers {
	assignmentType: AssignmentType;
	customEmail?: string;
}

interface ActivityPromptAnswers {
...
export async function configureTasks(): Promise<TaskDefinition[]> {
	...
	while (addMore) {
		...
		const task = await inquirer.prompt<TaskDefinition>([
			{
				type: "input",
				name: "id",
		...
		const { activity, customActivity } =
			await inquirer.prompt<ActivityPromptAnswers>([
				{
					type: ListType,
					name: "activity",
		...
		if (moreDetails.assignTask) {
			const { assignmentType, customEmail } =
				await inquirer.prompt<AssignmentPromptAnswers>([
					{
						type: ListType,
						name: "assignmentType",
						message: "Assignment:",
					...
					},
					{
						...
						when: (answers) => answers.assignmentType === "custom",
						validate: (input: string) => {
							if (!input || input.trim() === "") {
		...
		if (advancedOptions) {
			const advanced = await inquirer.prompt([
				...
				},
				{
					type: "input",
					name: "condition",
					//biome-ignore lint: The dynamic string is intended here
 ...
 */
async function configureTaskCustomFields(): Promise<Record<string, unknown>> {
	...
	while (addMore) {
		...
		let parsedValue: unknown = field.fieldValue;
		if (!Number.isNaN(Number(field.fieldValue))) {
