  ...
  if (geminiKey && ollamaAvailable) {
    const provider = assertNotCancelled(
      await select({
        ...
        options: [
          { label: " Google Gemini (Cloud, Free tier)", value: "gemini" },
          { label: "  Ollama (Local, Completely free)", value: "ollama" },
        ],
      }),
    );

    return createConfig(provider as AIProvider, geminiKey);
  }

  if (geminiKey) {
    console.log(chalk.cyan("Using Google Gemini AI"));

    const model = assertNotCancelled(
      await select({
    ...
    return createConfig("gemini", geminiKey, undefined, model as string);
  }
    ...
    .with("gemini", async () => {
      {
        ...
        if (!geminiKey) {
          ...
          const provideKey = assertNotCancelled(
            await confirm({
        ...
        if (!modelName) {
          const model = assertNotCancelled(
            await select({
              ...
              options: [
                {
                  label: "Gemini 2.0 Flash (Recommended - Fast & Free)",
                ...
                {
                  label: "Gemini 1.5 Pro (More capable)",
