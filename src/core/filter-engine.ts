...
import { WorkItemType } from "@/platforms";
...
export class FilterEngine {
  ...
  convertFilter(templateFilter: TemplateFilter): PlatformFilter {
    logger.debug("FilterEngine: Converting template filter to platform filter");
    ...
    logger.debug("FilterEngine: Conversion complete", {
      workItemTypes: platformFilter.workItemTypes?.length || 0,
      states: platformFilter.states?.length || 0,
      hasTags: !!platformFilter.tags,
      excludeIfHasTasks: platformFilter.excludeIfHasTasks,
    });
